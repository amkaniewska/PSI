...
findAssocs(tdm,"poor",0.5)
findAssocs(tdm,"stewardess",0.5)
...
# Wytypowane słowo i próg asocjacji
target_word_2 <- "stewardess"
cor_limit <- 0.4


# Oblicz asocjacje dla tego słowa
associations <- findAssocs(tdm, target_word_2, corlimit = cor_limit)
assoc_vector <- associations[[target_word_2]]
assoc_sorted <- sort(assoc_vector, decreasing = TRUE)
...
ggplot(assoc_df, aes(x = score, y = reorder(word, score))) +
  ...
  theme_minimal(base_size = 12) +
  labs(
    title = paste0("Asocjacje z terminem: '", target_word_2, "'"),
    subtitle = paste0("Próg r ≥ ", cor_limit),
    x = "Współczynnik korelacji Pearsona",
    y = "Słowo"
  ) +
